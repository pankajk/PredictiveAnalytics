...
app.title ='Visualization Tool for Gender Gap'
...
#Load in data:
data_sf = pd.read_csv('../data/wagegap/cleaned_sf.csv')
data_nb = pd.read_csv('../data/wagegap/cleaned_nb.csv')
...
app.layout = html.Div(
  ...
  children=[
    ...
    html.Div(className='col-lg-2 p-3 mb-2 bg-light text-dark', 
             children=[
                     html.Div(dcc.Markdown('### Gender Gap Visualization App')),                     
                     html.Div(dcc.Markdown('''---''')),
                     html.Div(dcc.Markdown('''Based on the dataset on found [here](https://transparentcalifornia.com/).''')),
                     html.Div(dcc.Markdown('''For the full step by step analysis and preprocessing of this dataset click [here](https://github.com/pankajk/PredictiveAnalytics).''')),
                     html.Div(dcc.Markdown('''---''')),                    
                     html.Div(className='',
                              children=[html.B('Job Category:'),
                     dcc.Dropdown(
                             id='input1',
